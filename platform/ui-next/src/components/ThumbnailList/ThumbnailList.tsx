      ...
      >
        {thumbnailItems.length > 0 && (
          <div
            ...
            className="bg-background grid grid-cols-[repeat(auto-fit,_minmax(118px,1fr))] place-items-start gap-[6px]"
          >
        ...
        {listItems.length > 0 && (
          <div
            id="ohif-thumbnail-list"
            className="bg-background grid grid-cols-[repeat(auto-fit,_minmax(0,250px))] place-items-start gap-[4px]"
          >
            {listItems.map(item => {
              const { displaySetInstanceUID, componentType, numInstances, ...rest } = item;
              const isActive = activeDisplaySetInstanceUIDs.includes(displaySetInstanceUID);
